...
public final class CMYKFileProcessor {
	 ...
	 * Constructor
	 */
	public CMYKFileProcessor(File imgFile, String iccPath, boolean actual_size, int paper_size, int ppi, Yield yield, boolean grayScale) throws Exception{
		try{
	...
	private void calculateTotalInkCostPerA4(Yield yield){
		float cyanTimes = CMYK[C].floatValue()/DEFAULT_COVERAGE_PER_COLOR;
		float cyanPages = yield.getCyanYieldPage()/cyanTimes;
		float cyanInkCost = yield.getCyanInkPrice()/cyanPages;
		//System.out.println("Cyan Ink Cost Per A4: " + cyanInkCost);
		float magentaTimes = CMYK[M].floatValue()/DEFAULT_COVERAGE_PER_COLOR;
		float magentaPages = yield.getMagentaYieldPage()/magentaTimes;
		float magentaInkCost = yield.getMagentaInkPrice()/magentaPages;
		//System.out.println("Magenta Ink Cost Per A4: " + magentaInkCost);
		float yellowTimes = CMYK[Y].floatValue()/DEFAULT_COVERAGE_PER_COLOR;
		float yellowPages = yield.getYellowYieldPage()/yellowTimes;
		float yellowInkCost = yield.getYellowInkPrice()/yellowPages;
		//System.out.println("Yellow Ink Cost Per A4: " + yellowInkCost);
		float blackTimes = CMYK[K].floatValue()/DEFAULT_COVERAGE_PER_COLOR;
		float blackPages = yield.getBlackYieldPage()/blackTimes;
		float blackInkCost = yield.getBlackInkPrice()/blackPages;
